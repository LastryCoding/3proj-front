...
export default function SignInSide() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const loginRes = await post(`auth/login`, loginUser);
      if (loginRes.status === 200) {
        setUserData({
          token: loginRes.message.token,
          user: loginRes.message.user,
        });
        localStorage.setItem("auth-token", loginRes.message.token);
        history.push("/");
      } else if (loginRes.status === 400) {
        setSignInErrors({ ...loginRes.message });
      } else {
